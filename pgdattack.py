...
if __name__ == "__main__":
    ...
    if sequence_tensor is not None:
        ...
        # PGD params
        epsilon = 0.03          # Maximum total perturbation
        ...
        for i in range(num_steps):
            ...
            # calculate the loss between the two single values
            ...
            # Create the perturbed image by taking a small step
            ...
            # Clip the image to ensure it remains in a valid range [-1, 1]
            adversarial_input = torch.clamp(adversarial_input, -1, 1)

        print("Adversarial input generated via PGD.")

        output_folder = r'C:\Users\Deborshi Chakrabarti\Desktop\advattk\attack_results_pgd'
        os.makedirs(output_folder, exist_ok=True)
        
        adversarial_frames_tensor = adversarial_input.squeeze(0)
        
        adversarial_frames_tensor = adversarial_frames_tensor * 0.5 + 0.5 
        to_pil_image = transforms.ToPILImage()
        
        num_frames = adversarial_frames_tensor.shape[3]
        ...
        if abs(original_pred.mean().item() - adversarial_pred.mean().item()) > epsilon:
